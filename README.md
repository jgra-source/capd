...
- **v1.0.9** — Removed the **Diagnostics** panel, along with the raw-body dump and its copy buttons. It put account internals — organization UUID, billing endpoint payloads, subscription fields — one click away in the everyday UI, which is more exposure than a usage meter needs. Nothing about capture or parsing changed; the same data is still in `chrome.storage.local` and readable from the service worker console (see CLAUDE.md → *Debugging when readings stop*).
- **v1.0.8** — Two fixes.
  ...
  - **Dropped the `overage_spend_limit` and `usage` cards.** They printed their endpoints' top-level scalars, which are organization UUIDs, internal flags, `member_dashboard_available`, and a `monthly_credit_limit` matching nothing the account displays — nothing a user can act on.
- **v1.0.7** — Account cards are now readable rather than raw.
  - **Money is formatted as currency, but only where the unit is provable.** Claude's billing API mixes scales inside a single body: `credits` returns `amount` in minor units (corroborated by the sibling `promo_tranches[].remaining_amount_minor_units`, which tracks it to within a unit) directly alongside `balance_credits`, which is *already* in whole units and reads as roughly one-hundredth of it. So a money-sounding name proves nothing about scale, and dividing on name alone rendered a $69 balance as $0.69. Conversion is now limited to keys that state their own unit (`*_cents`, `*_minor`, `*_minor_units`), plus the bare `amount` field when a sibling `currency` confirms the body is describing money. Ambiguous fields (`balance_credits`, `monthly_credit_limit`, `used_credits`) are printed exactly as sent.
  ...
  - Every reformatted value keeps the untouched API value in a hover tooltip (`raw: <original>`), so a conversion can always be checked against what Claude actually sent.
  - `monthly_credit_limit` is **not** converted, and the reason is worth recording. It was briefly added on the reasoning that it shares the `overage_spend_limit` body with `used_credits`, which *is* provably minor units (it is byte-identical to `spend.used.amount_minor`, a field tagged `{currency: "USD", exponent: 2}`). Converting it produced a plausible figure matching no limit the account displays — the real one is asserted independently by four fields that all agree: `spend.limit.amount_minor`, `extra_usage.monthly_limit`, `overage_credit_grant.amount_minor_units`, and the promo tranche's `granted_amount_minor_units`. `monthly_credit_limit` is a *disabled*, organization-scoped setting describing something else. Sharing a body is proximity, not proof.
