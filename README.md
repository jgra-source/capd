...
- **v1.0.7** — Account cards are now readable rather than raw.
  ...
  - `monthly_credit_limit` is **not** converted, and the reason is worth recording. It was briefly added on the reasoning that it shares the `overage_spend_limit` body with `used_credits`, which *is* provably minor units (it reads `380`, byte-identical to `spend.used.amount_minor: 380` tagged `{currency: "USD", exponent: 2}`). That rendered `5000` as `$50.00` — but the account's real monthly spend limit is `$100.00`, asserted independently by four fields that all read `10000`: `spend.limit.amount_minor`, `extra_usage.monthly_limit`, `overage_credit_grant.amount_minor_units`, and the promo tranche's `granted_amount_minor_units`. `monthly_credit_limit` is a *disabled*, organization-scoped setting describing something else. Sharing a body is proximity, not proof.
