...
async function updateBadge(windows) {
  if (!windows) {
    const store = await sget(["rl"]);
    windows = (store.rl && store.rl.windows) || {};
  }
...
async function getState() {
  const store = await sget(["rl", "bodies", "history", "settings"]);
  const windows = (store.rl && store.rl.windows) || {};
  return {
    rl: store.rl || null,
    ...
    updatedAt: store.rl ? store.rl.updatedAt : null,
  };
}
